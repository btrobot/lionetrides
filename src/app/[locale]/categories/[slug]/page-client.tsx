...
export default function CategoryDetailPage() {
  ...
  useEffect(() => {
    ...
    async function fetchCategory() {
      try {
        const res = await fetch('/api/v1/categories?pageSize=100');
        const data = await res.json();
        const items = data.data?.items || [];
        const found = items.find((c: Category) => c.slug === slug);
        if (found) {
          setCategory(found);
        } else {
          setNotFound(true);
  ...
  // Fetch products when category is found or page changes
  useEffect(() => {
    if (!category) return;
    const cat = category;
    ...
    fetchProducts();
  }, [category, currentPage, category?.id]);
      ...
      <div className="bg-gradient-to-br from-blue-900 via-blue-700 to-indigo-900 text-white">
        <div className="max-w-7xl mx-auto px-4 py-16">
          ...
          <div className="flex items-center gap-6">
            {category.image_url ? (
              <div className="relative w-24 h-24 rounded-2xl overflow-hidden bg-white/10 shrink-0">
                <Image
            ...
            <div>
              ...
              {category.description && (
                <p className="text-xl text-blue-200 max-w-2xl">{category.description}</p>
              )}
              <p className="text-blue-300 text-sm mt-2">
                {total > 0 ? `${total} products in this category` : 'Loading products...'}
              </p>
            </div>
          </div>
        </div>
