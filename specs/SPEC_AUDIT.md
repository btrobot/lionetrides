...
| # | 问题 | 影响 | 状态 |
|---|------|------|------|
| 11 | 字段类型用 `varchar(50)` 替代 `enum` | 失去类型安全 | ❌ 待处理 |
| 12 | 缺少 `CustomerAddress` 表 | 客户地址管理功能缺失 | ✅ 新增表 |
| 13 | 缺少 `ReviewImage` 表 | 评价无法上传图片 | ✅ 新增表 |
| 14 | 缺少 `TeamMember` 表 | 关于我们-团队展示功能缺失 | ✅ 新增表 |
| 15 | 缺少 `publishedAt` 字段 | 新闻发布时间无法精确控制 | ❌ 待处理 |
| 16 | 登录错误 5 次锁定 1 小时 | 账户安全 | ✅ 已实现 |
