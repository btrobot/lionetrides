...
async function seed() {
  ...
  try {
    ...
    // 插入品牌
    await pool.query(`
      INSERT INTO brands (name, description, country, is_active) VALUES
        ('Lionet Rides', 'Premium amusement ride manufacturer since 1995', 'CN', true),
        ('ThemeCo', 'International theme park solutions provider', 'US', true)
    `);
    console.log('✓ 2 brands seeded');

    // 插入示例产品
    await pool.query(`
      INSERT INTO products (name, slug, description, category_id, brand_id, price_range, status) VALUES
        ('Thunder Bolt Coaster', 'thunder-bolt-coaster', 'A thrilling high-speed roller coaster with multiple loops and drops', 1, 1, '$500,000 - $2,000,000', 'active'),
        ('Sky Wheel 60m', 'sky-wheel-60m', '60-meter panoramic Ferris wheel with 36 climate-controlled cabins', 2, 1, '$800,000 - $3,000,000', 'active'),
        ('Royal Carousel', 'royal-carousel', 'Classic double-decker carousel with 60 horses and LED lighting', 3, 1, '$200,000 - $800,000', 'active')
    `);
